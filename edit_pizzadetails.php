<html>
	<head>
		<title>EDIT PIZZA DETAILS</title>
		<style>
	html,body{
		height:98%;
		}
...
.head1{
	...
	<body>
		<div class="head1">
		<h1 class="head1" style="color:white">EDIT PIZZA DETAILS</h1>
		...
		<div>
			<form method="post">
				<h2 class="text1" style="color: white"><u>ADD PIZZA ITEM</u></h2>
		...
		if($conn->connect_error){
			...
			if(isset($_POST['submit1'])){
				...
				$info = $_POST['info'];
				$price = $_POST['price'];
				$stmt = $conn->prepare("insert into pizza_menu(id,name,info,big) values(?,?,?,?)");
		...
		if($conn->connect_error){
			...
			if(isset($_POST['submit2'])){
				...
				$stmt = $conn->prepare("delete from pizza_menu where id = ?");
		...
		if($conn->connect_error){
			...
			if(isset($_POST['sub'])){
				...
				$stmt = $conn->prepare("update pizza_menu set big=? where id= ?");
