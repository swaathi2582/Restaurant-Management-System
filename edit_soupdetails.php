...
.head1{
	...
	<body>
		<div class="head1">
		<h1 class="head1" style="color:#fff">EDIT SOUP DETAILS</h1>
		...
		<div>
			<form method="post">
				<h2 class="text1" style="color: white"><u>ADD SOUP ITEM</u></h2>
		...
		if($conn->connect_error){
			...
			if(isset($_POST['submit1'])){
				...
				$info = $_POST['info'];
				$price = $_POST['price'];
				$stmt = $conn->prepare("insert into soup_menu(id,name,info,price) values(?,?,?,?)");
		...
		if($conn->connect_error){
			...
			if(isset($_POST['submit2'])){
				...
				$stmt = $conn->prepare("delete from soup_menu where id = ?");
		...
		if($conn->connect_error){
			...
			if(isset($_POST['sub'])){
				...
				$stmt = $conn->prepare("update soup_menu set price=? where id= ?");
